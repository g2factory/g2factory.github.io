...
class BlogRepository {
  ...
  Future<List<Map<String, String>>> loadBlogHeadlineList({int offset = 0, int limit = 10}) async {
    List<Map<String, String>> blogHeadlineList = List<Map<String, String>>();
    List<String> blogPaths = await _loadBlogPathList(offset: offset, limit: limit);
    await Future.forEach(blogPaths, (path) async {
      Iterable<Match> blogMatch = RegExp(r"[^#\s][^\n<]+").allMatches(await loadBlog(path));
  ...
  Future<List<String>> _loadBlogPathList({int offset = 0, int limit = 10}) async {
    ...
    return _blogPathList = blogPaths.skip(offset).take(min(limit, blogPaths.length)).toList();
  }
}